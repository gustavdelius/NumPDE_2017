...
Once the calculation completes you will see that the error in the backward difference method follows a line of slope -1 on the log-log plot, meaning that the error goes down as $1/N$, whereas the error in the Crank-Nicolson method follows a line of slope -2, meaning that the error goes down as $1/N^2$. 

Just out of interest I now also make a plot of the error as a function of $x$ and $t$, rather than just looking at the maximum error.

```{r}
plotError <- function(N, M, method) {
    # numerical solution
    numSol <- method(M=M, N=N, T=0.2)
    # exact solution
    x <- numSol$x
    t <- numSol$t
    xy <- mesh(x, t)
    ...
    persp3D(x, t, u - numSol$w,
        xlab="x", ylab="t", zlab="Error",
        ticktype="detailed", nticks=4)
}
```

```{r}
plotError(60, 60, backwardDifference)
```
```{r}
