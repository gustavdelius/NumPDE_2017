...
Here, $r$ is the growth rate of susceptibles and $k$ their carrying capacity in the absence of infective. $\alpha$ is the rate in which susceptibles get infected due to cantact with infectives, $a$ is a half saturation constant, and $\delta$ is the recover rate from an infection.
With the second derivative terms we take the spartial dispersion of individuals into account. 
...
Now we introduce the meshgrid. Therefore we use the same stepsize, and the same amount of steps in $x-$ and $y-$ direction.
```{r}
h<-0.1
N<-200
x<-h*(0:N)
...
However, to make the code more efficient, we will save results just for a subset of all the results.
```{r}
tau<-h^2/6
M<-1000
M1<-50
...
Beside the trivial solution $I=0, S=0$ the system has a second steady-state solution, that we copmpute by setting all derivates equal 0.
```{r}
...
We take this steady-state solution as constant initial condition, except from one area, where we introduvce a peak of infectives.
```{r}
