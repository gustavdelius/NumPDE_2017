...
In this Mini-Project, we present the following equation and conditions, as well as attempt to recreate the figures shown in the original paper "Boltzmann and Fokker-Planck equations modelling opinion formation in the presence of strong leaders" by Bertram Düring, Peter Markowich, Jan-Frederik Pietschmann, and Marie-Therese Wolfram. We focus specifically on Figure 3.

![Figure 3 from "Boltzmann and Fokker-Planck equations modelling opinion formation in the presence of strong leaders". In order, a) is the distribution of opinion followers, b) is the distribution of opinion leaders, c) and d) are the steady state distributions of the same. The shaded regions arose due in Monte-Carlo Simulation.](BrennenFig.jpg)
...
$$
$$
\partial_{\tau} g_{1}(w,\tau)= \partial_{w}\left(\left(\frac{1}{\tau_{1,1}} \mathcal{K}_1 (w,\tau) + \frac{1}{2 \tau_{1,2}} \mathcal{K}_3(w,\tau)\right)g_1(w,\tau)\right) + \left(\frac{\lambda_{1,1} M_1}{2 \tau_{1,1}} + \frac{\lambda_{1,2} M_2}{4 \tau_{1,2}}\right) \partial^2_{ww} \left(D^2_1 (w) g_1 (w,\tau) \right)
$$

$$
\partial_{\tau} g_{2}(w,\tau)= \partial_{w}\left(\frac{1}{\tau_{2,2}} \mathcal{K}_2 (w,\tau) g_2(w,\tau)\right) + \frac{\lambda_{2,2} M_2}{2 \tau_{2,2}} \partial^2_{ww} \left(D^2_2 (w) g_2 (w,\tau) \right)
$$

where $M_i = \int g_i dv$. This system is a Fokker-Planck system in which there are two simultaneously interacting populations. The first population, described by $g_1$, represents opinion followers, whose interactions influence each other. The second population, described by $g_2$, represents opinion leaders, whose interactions influence both each other and the followers. The above system uses the distribution functions $g_1, g_2$ over the space of possible opinions $w$ to track the evolution of the distributions of followers and leaders near the steady-state. Time is described by the variable $\tau$. The interactions between the populations are described by the functions $\mathcal{K}_i, D_1,$ and $D_2$. The first of these is the drift operator caused by interactions between the differing opinions of the population and is a function of the ability to reach compromise, itself described as the functions $P_i$ below.
$$
\mathcal{K}_i(w,\tau) = \int_\mathcal{I}P_i(|w-v|)(w-v)g_i(v,\tau)dv \text{ for } i = 1,2
$$
$$
\mathcal{K}_3(w,\tau) = \int_\mathcal{I}P_3(|w-v|)(w-v)g_2(v,\tau)dv.
$$
...
**GWD: I do not see why the time derivative would be zero at the boundary. I think quite to the contrary, the requirement that now probability flows outside the interval means that the boundary values of the density have to be allowed to be time-dependent. Now probably in your application it does not make much difference because the probability density will be close to zero at the boundary anyway.**
...
**GWD: I am pleased you noticed their mistake. However your formula for the Gaussian is also not correct. In the exponent you should be dividing by the square of the standard deviation.**
...
Next, we shall handle the term $\partial_{w}(\frac{1}{\tau_{2,2}} \mathcal{K}_2 (w,\tau) g_2(w,t))$. Then we will extend the treatment given here to cover the term $\partial_{w}((\frac{1}{\tau_{1,1}} \mathcal{K}_1 (w,\tau) + \frac{1}{2 \tau_{1,2}} \mathcal{K}_3(w,\tau))g_1(w,t))$. Given that $\tau_{2,2}$ is constant, we need only consider the functions.
...
\end{align}
$$
**GWD: I am surprised that you are using this rather complicated scheme with truncation error of $O(\omega)$. Why not simply use the central difference formula
$$
\partial_{w}( \mathcal{K}_2 (w,\tau) g_2(w,t))=\frac{\mathcal{K}_2(x_k+\omega,t_j)\gamma_{2,k+1,j}-\mathcal{K}_2(x_k-\omega,t_j)\gamma_{2,k-1,j}}{2\omega}
$$
which has truncation error of $O(\omega^2)$?**
...
\end{align}
$$
**Again, why so complicated? Why not simply use central difference?**
...
```
**GWD: It seems you are using D instead of D^2 in the above code.**

**GWD: One important remark about your code: when using a vector-oriented language (like for example R, python with numpy, matlab), you should do calculations with entire vectors rather than looping through the components of the vector and doing the calculation separately for each component. That is both more concise and much faster. Below is an example of code for the forward-difference method that looks much simpler and runs much faster and produces results similar to those in the paper. I simply adapted the code from Lab 11 a bit. I also use a clever way to evaluate the convolution integrals using Fourier transforms (look up "convolution theorem" in wikipedia).**

```{r}
K <- function(x, g, r) {
    Re(fft(fft(P(x, 0, r)*x)*fft(g), inverse = TRUE))*2/length(x)
}
...
```
It is clear that there is scope for improvement in precision but it is a good first shot.
