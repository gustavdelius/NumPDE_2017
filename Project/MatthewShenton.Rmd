...
Which is exactly what [1] tells us it should look like when using 1-soliton solutions and no-flux boundary conditions.

This particular numerical scheme was fairly simple to create, however there are some quite major shortcomings. The explicit method is unstable if $h<\tau$, limiting out choices for $M,N$ and $T$. Also the error in this method is calculated as $O(\tau^2+h^2)$, which can be improved upon e,g, $O(\tau^4+h^4)$ can be achieved. For a standard computer to run the code above took on average 5-10 minutes, for smaller time steps this would take exponentially longer, so if one wished for greater accuracy this method would not be recommended.
